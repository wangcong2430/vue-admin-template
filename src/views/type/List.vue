...
<script>
import { getList, del, updata } from "@/api/type"; //导入调用获取菜品分类列表，删除菜品分类，修改菜品分类请求的方法
...
export default {
  ...
  mounted() {
    getList().then((res) => { //调用获取菜品分类列表请求的方法发送请求     
      console.log(res);
      this.list = res.data;
    });
  },
  methods: {
    del(index, row,) {
      ...
      this.$confirm("确定真的删除该菜品分类?", "提示", {
        ...
        .then(() => {
          ...
          del({ id: row._id }).then((res) => {//调用删除菜品分类请求的方法发送请求  
            console.log(res);
          });
          this.$message({
            type: "success",
    ...
    sure() {
      updata({ id: this.id, index: this.index, name: this.name }).then( //调用修改菜品分类请求的方法发送请求  
        (res) => {
          console.log(res);
          this.$message({
            type: "success",
